...
file_path=r"C:\Users\Thien Dang\Desktop\ins\training\training\04_Greasewood_Solar\Calculation_Latest-2024.xlsm"
...
for i in range(1,dem[1]+1):
    sheet1=data_excel[f'1 General({i})']
...
for i in range (1,dem[2]+1):
    sheet2=data_excel[f'2 XFMR Impedance({i})']
    v_110003=sheet2['G5'].value
    psspy.bus_chng_3(100000+i*10000+3,[_i,_i,_i,_i],[v_110003,_f,_f,_f,_f,_f,_f])
...
# Đếm số mpt
ierr, bus_names = psspy.abuschar(sid=-1, flag=2, string=["NAME"])
dem2=0
for bus_name in bus_names[0]:
    if 'MPT' in bus_name and 'SEC' in bus_name:
        dem2 +=1
...
def mpt_data(mpt,s2):
    if mpt==s2:
        ...
        sheet2=data_excel['2 XFMR Impedance(1)']
        r12=sheet2['D12'].value
        x12=sheet2['D13'].value
        r23=sheet2['H12'].value
        x23=sheet2['H13'].value
...
# Nhập branch sheet 4
sheet4=data_excel['4 UG collection sys impedance']
for i in range(1,1000):
    if sheet4[f'AH{i}'].value=='Equivalent R (pu)':
        j=i+1
        k=1
        while sheet4[f'AH{(j)}'].value not in (None, ''):
            ...
            psspy.branch_chng_3(10000+1000*k,100000+10000*k,r"""1""",[_i,_i,_i,_i,_i,_i],[r,x,b,_f,_f,_f,_f,_f,_f,_f,_f,_f],[rate1,rate2,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f],_s)
            psspy.seq_branch_data_3(10000+1000*k,100000+10000*k,r"""1""",_i,[r0,x0,b0,_f,_f,_f,_f,_f])
            k += 1
            j += 1

# Nhập gen
sheet5=data_excel['5 Generator Impedance']
r=sheet5['B13'].value
x=sheet5['B14'].value
